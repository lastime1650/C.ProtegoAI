#include "PNP_mount_management.h"
	...
	if (Get_USB_Serial(&USB_DEVICE_SYMBOLIC_STRING, &get_SERIAL_NUM, 2, FALSE)) {
		...
		while (output == NULL) {
			...
			if (is_unmount == FALSE) {
				...
				output = Finder_ALL_DEVICE_DRIVES_Node(ALL_DEVICE_DRIVES_Start_Node, NULL, &get_SERIAL_NUM, NULL);
			}
...
NTSTATUS PNP_Register(PDRIVER_OBJECT DriverObject) {
	...
	return IoRegisterPlugPlayNotification(
		...
		DriverObject,
		(PDRIVER_NOTIFICATION_CALLBACK_ROUTINE)MyNotificationRoutine,
		NULL,
		&notificationHandle
	);
...
NTSTATUS MyNotificationRoutine(
...
)
{
	...
	if (NT_SUCCESS(status)) {
		...
		if (NT_SUCCESS(status)) {
			...
			if (IsEqualGUID(&NotificationStructure->Event, &GUID_DEVICE_INTERFACE_ARRIVAL)) {
			...
			else if (IsEqualGUID(&NotificationStructure->Event, &GUID_DEVICE_INTERFACE_REMOVAL)) {
