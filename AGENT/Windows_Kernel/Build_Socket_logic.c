...
/*TCP 송-수신*/
NTSTATUS Send_TO_TCP_Server(PVOID Buffer, SIZE_T Buffer_size, SOCKET_INFORMATION send_or_receive, PKSOCKET* NewSocket_parm) {
...
NTSTATUS Make_TCP_Connection(_In_ PCHAR Server_ip, _In_ USHORT Server_port, PKSOCKET* NewSocket_parm) { // 초기화 된 소켓을 포인터로 받아서 수정 작업을 함. 
    ...
    *NewSocket_parm = ExAllocatePoolWithTag(NonPagedPool, sizeof(KSOCKET), 'ABCD');
    ...
    PCHAR ip_str = ExAllocatePoolWithTag(NonPagedPool, sizeof("000.000.000.000"), 'zxcv');
    strcpy_s(ip_str, 14, Server_ip);
    PCHAR Next_str;
    PCHAR str_token = strtok_s(ip_str, ".", &Next_str);
    ULONG ip_byte[4] = { 0 };
    int i = 0;
    while (str_token != NULL && i < 4) {
        DbgPrintEx(DPFLTR_IHVDRIVER_ID, DPFLTR_ERROR_LEVEL, "문자 -> %s ", str_token);
